...
-- Votre nom: wissem azabi                       Votre DA: 6240576
...
--Description de la table des usagers
DESCRIBE OUTILS_USAGER;

-- Description de la table des villes
DESCRIBE OUTILS_VILLE;

--Description de la table des outils
...
FROM OUTILS_OUTIL
WHERE UPPER(SUBSTR(COULEUR, 1, 1)) = 'J';
...
SELECT CONCAT(OU.PRENOM, ' ', OU.NOM_FAMILLE) AS Nom_Complet_Usager, OE.NUM_EMPRUNT, 
       OE.DATE_RETOUR - OE.DATE_EMPRUNT AS Duree_Emprunt, COALESCE(OO.PRIX, 0) AS Prix_Outil
FROM OUTILS_EMPRUNT OE
JOIN OUTILS_USAGER OU ON OE.NUM_USAGER = OU.NUM_USAGER
LEFT JOIN OUTILS_OUTIL OO ON OE.CODE_OUTIL = OO.CODE_OUTIL
...
SELECT PRENOM, NOM_FAMILLE, COURRIEL
FROM OUTILS_USAGER
...
SELECT OO.CODE_OUTIL, COALESCE(OO.PRIX, 0) AS Prix
FROM OUTILS_OUTIL OO
LEFT JOIN OUTILS_EMPRUNT OE ON OO.CODE_OUTIL = OE.CODE_OUTIL
...
SELECT OO.NOM, COALESCE(OO.PRIX, AVG(OO.PRIX) OVER ()) AS Prix
FROM OUTILS_OUTIL OO
WHERE FABRICANT = 'Makita'
AND OO.PRIX > AVG(OO.PRIX) OVER ();
...
SELECT U.PRENOM, U.NOM_FAMILLE, U.ADRESSE, E.CODE_OUTIL, E.NUM_EMPRUNT
FROM OUTILS_USAGER U
JOIN OUTILS_EMPRUNT E ON U.NUM_USAGER = E.NUM_USAGER
WHERE YEAR(E.DATE_EMPRUNT) > 2014
...
INSERT INTO OUTILS_OUTIL (CODE_OUTIL, NOM, COULEUR, ANNEE, FABRICANT, PRIX)
VALUES (1, 'Scie circulaire', 'Bleue', 2023, 'Makita', 149.99);
